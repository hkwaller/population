 ...
 * **start and end countries** (per the game's request), so the player always sees
 * the two anchors and the terrain between them. Start/end are colour-coded and each
 * intermediate country the player locks in lights up in order, with a dashed line
 * threading the chain together. Purely presentational - no pan/zoom.
 */
export function RouteMap({
  ...
  return (
    ...
    >
      ...
      {/* Legend */}
      <div
        style={{ position: 'absolute', left: 8, bottom: 8, display: 'flex', gap: 10 }}
        className="pointer-events-none select-none rounded-pill bg-white/85 px-3 py-1 text-[11px] font-black text-pop-ink"
      >
